...
const makeChatsSocket = (config) => {
  ...
  const fetchPrivacySettings = async (force = false) => {
    if (!privacySettings || force) {
      ...
      privacySettings = WABinary_1.reduceBinaryNodeToDictionary(
        content?.[0],
        "category",
      );
    }
  ...
  const getBotListV2 = async () => {
    ...
    const botList = [];
    for (const section of WABinary_1.getBinaryNodeChildren(
      botNode,
      "section",
    )) {
  ...
  /** update the profile picture for yourself or a group */
    ...
    const { img } = await Utils_1.generateProfilePicture(content, dimensions)
    await query({
      tag: "iq",
      attrs: {
        target: targetJid,
  ...
  const fetchBlocklist = async () => {
    ...
    return WABinary_1.getBinaryNodeChildren(listNode, "item").map(
      (n) => n.attrs.jid,
    );
  };
  ...
  const getBusinessProfile = async (jid) => {
    ...
    const profileNode = WABinary_1.getBinaryNodeChild(
      results,
      "business_profile",
    );
    ...
    if (profiles) {
      ...
      const businessHours = WABinary_1.getBinaryNodeChild(
        profiles,
        "business_hours",
      );
    ...
    async (collections, isInitialSync) => {
      ...
      await authState.keys.transaction(async () => {
        ...
        while (collectionsToHandle.size) {
          ...
          const decoded = await Utils_1.extractSyncdPatches(
            result,
            config?.options,
          );
