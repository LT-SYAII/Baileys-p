...
const makeGroupsSocket = (config) => {
  ...
  return {
    ...
    groupRequestParticipantsList: async (jid) => {
      ...
      const node = WABinary_1.getBinaryNodeChild(
        result,
...
const extractGroupMetadata = (result) => {
  ...
  const eph = WABinary_1.getBinaryNodeChild(group, "ephemeral")?.attrs
    .expiration;
  const memberAddMode =
    WABinary_1.getBinaryNodeChildString(group, "member_add_mode") ===
    "all_member_add";
  const metadata = {
    id: groupId,
    addressingMode: mode,
    ...
    owner: group.attrs.creator
      ? WABinary_1.jidNormalizedUser(
          mode === "lid" ? group.attrs.creator_pn : group.attrs.creator,
        )
      : undefined,
    ...
    descId,
    linkedParent:
      WABinary_1.getBinaryNodeChild(group, "linked_parent")?.attrs.jid ||
      undefined,
    ...
    isCommunityAnnounce: !!WABinary_1.getBinaryNodeChild(
      group,
      "default_sub_group",
    ),
    joinApprovalMode: !!WABinary_1.getBinaryNodeChild(
      group,
      "membership_approval_mode",
    ),
    memberAddMode,
    participants: WABinary_1.getBinaryNodeChildren(group, "participant").map(
      ({ attrs }) => {
        return {
