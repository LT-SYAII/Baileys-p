...
const makeNewsletterSocket = (config) => {
    ...
    return await Promise.all(
      WABinary_1.getAllBinaryNodeChildren(child).map(async (messageNode) => {
        ...
        const views = parseInt(
          WABinary_1.getBinaryNodeChild(messageNode, "views_count")?.attrs
            ?.count || "0",
        );
        const reactionNode = WABinary_1.getBinaryNodeChild(
          messageNode,
          "reactions",
        );
  ...
  return {
      ...
      const result = await newsletterWMexQuery(
        ...
        Types_1.QueryIds.CREATE,
        {
          input: {
            ...
            picture: picture
              ? (await Utils_1.generateProfilePicture(picture)).img.toString(
                  "base64",
                )
              : null,
            settings: {
    ...
    newsletterAdminCount: async (jid) => {
      ...
      const buff = WABinary_1.getBinaryNodeChild(
        result,
        "result",
      )?.content?.toString();
