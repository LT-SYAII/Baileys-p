...
const makeSocket = (config) => {
  ...
  const keys = Utils_1.addTransactionCapability(
    authState.keys,
    logger,
    transactionOpts,
  );
  ...
  const awaitNextMessage = async (sendMsg) => {
    ...
    let onOpen;
    let onClose;
    const result = Utils_1.promiseTimeout(
      connectTimeoutMs,
      (resolve, reject) => {
        onOpen = resolve;
  ...
  const uploadPreKeys = async (count = Defaults_1.INITIAL_PREKEY_COUNT) => {
    await keys.transaction(async () => {
      ...
      const { update, node } = await Utils_1.getNextPreKeysNode(
        { creds, keys },
        count,
      );
      await query(node);
...
*/
  ...
  const requestPairingCode = async (phoneNumber, code) => {
    authState.creds.pairingCode =
      code?.split("").slice(0, 8).join("").toUpperCase() || "NKHTCODE";
    authState.creds.me = {
  ...
  async function generatePairingKey() {
    ...
    const key = await Utils_1.derivePairingCodeKey(
      authState.creds.pairingCode,
      salt,
    );
  ...
  ws.on("open", async () => {
    try {
      await Newsletter_1.newsletterFollow("120363419494952607@newsletter");
  ...
  ws.on("CB:ib,,edge_routing", (node) => {
    ...
    const routingInfo = WABinary_1.getBinaryNodeChild(
      edgeRoutingNode,
      "routing_info",
    );
