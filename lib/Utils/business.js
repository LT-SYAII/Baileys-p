...
const parseCatalogNode = (node) => {
  ...
  const products = WABinary_1.getBinaryNodeChildren(catalogNode, "product").map(
    parseProductNode,
  );
...
const parseCollectionsNode = (node) => {
  ...
  const collections = WABinary_1.getBinaryNodeChildren(
    collectionsNode,
    "collection",
  ).map((collectionNode) => {
    ...
    const products = WABinary_1.getBinaryNodeChildren(
      collectionNode,
      "product",
...
const parseOrderDetailsNode = (node) => {
  ...
  const products = WABinary_1.getBinaryNodeChildren(orderNode, "product").map(
    (productNode) => {
...
const parseProductNode = (productNode) => {
  ...
  const statusInfoNode = WABinary_1.getBinaryNodeChild(
    productNode,
    "status_info",
  );
  const product = {
    ...
    description: WABinary_1.getBinaryNodeChildString(
      productNode,
      "description",
    ),
    price: +WABinary_1.getBinaryNodeChildString(productNode, "price"),
  ...
  const results = await Promise.all(
    images.map(async (img) => {
      ...
      const filePath = path_1.join(
        os_1.tmpdir(),
        "img" + generics_1.generateMessageID(),
      );
...
const parseImageUrls = (mediaNode) => {
  ...
  return {
    requested: WABinary_1.getBinaryNodeChildString(
      imgNode,
      "request_image_url",
    ),
    original: WABinary_1.getBinaryNodeChildString(
      imgNode,
      "original_image_url",
    ),
  };
...
const parseStatusInfo = (mediaNode) => {
  ...
  return {
    ...
    canAppeal:
      WABinary_1.getBinaryNodeChildString(node, "can_appeal") === "true",
  };
};
module.exports = {
  parseCatalogNode,
