...
const extractSyncdPatches = async (result, options) => {
  ...
  const collectionNodes = WABinary_1.getBinaryNodeChildren(
    syncNode,
    "collection",
  );
  const final = {};
  await Promise.all(
    collectionNodes.map(async (collectionNode) => {
      const patchesNode = WABinary_1.getBinaryNodeChild(
        collectionNode,
        "patches",
      );
      ...
      const snapshotNode = WABinary_1.getBinaryNodeChild(
        collectionNode,
        "snapshot",
      );
      const syncds = [];
      ...
      messageRange = {
        ...
        messages: lastMessages?.length
          ? lastMessages.map((m) => {
              ...
              if (
                !m.messageTimestamp ||
                !generics_1.toNumber(m.messageTimestamp)
              ) {
                throw new boom_1.Boom(
              ...
              if (m.key.participant) {
                m.key.participant = WABinary_1.jidNormalizedUser(
                  m.key.participant,
                );
              }
              return m;
    ...
    ev.emit("chats.update", [
      {
        ...
        pinned: action.pinAction?.pinned
          ? generics_1.toNumber(action.timestamp)
          : null,
        conditional: getChatUpdateConditional(id, undefined),
      },
    ]);
