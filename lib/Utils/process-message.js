    ...
    (!!normalizedContent ||
      ...
      (REAL_MSG_REQ_ME_STUB_TYPES.has(message.messageStubType) &&
        message.messageStubParameters?.some((p) =>
          WABinary_1.areJidsSameUser(meId, p),
        ))) &&
    hasSomeContent &&
...
const processMessage = async (
  ...
  if (protocolMsg) {
    switch (protocolMsg.type) {
      case WAProto_1.proto.Message.ProtocolMessage.Type
        ...
        if (process) {
          ...
          const data =
            await history_1.downloadAndProcessHistorySyncNotification(
              histNotification,
              options,
            );
      ...
      case WAProto_1.proto.Message.ProtocolMessage.Type.EPHEMERAL_SETTING:
        Object.assign(chat, {
          ephemeralSettingTimestamp: generics_1.toNumber(
    ...
    if (pollMsg) {
      ...
      const pollCreatorJid = generics_1.getKeyAuthor(
        creationMsgKey,
        meIdNormalised,
      );
