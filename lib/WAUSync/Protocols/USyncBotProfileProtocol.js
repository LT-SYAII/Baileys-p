...
class USyncBotProfileProtocol {
  ...
  parser(node) {
    ...
    const prompts = [];
    if (commandsNode) {
      for (const command of WABinary_1.getBinaryNodeChildren(
        commandsNode,
        "command",
      )) {
        commands.push({
          ...
          description:
            WABinary_1.getBinaryNodeChildString(command, "description") || "",
        });
      }
    }
    if (promptsNode) {
      for (const prompt of WABinary_1.getBinaryNodeChildren(
        promptsNode,
        "prompt",
      )) {
        prompts.push(
    ...
    return {
      ...
      attributes:
        WABinary_1.getBinaryNodeChildString(profile, "attributes") || "",
      description:
        WABinary_1.getBinaryNodeChildString(profile, "description") || "",
